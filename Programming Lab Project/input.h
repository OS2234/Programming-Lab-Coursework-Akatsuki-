...
struct WordStructure TWStruct[MAX_LINES];
struct WordStructure WStruct[MAX_LINES];
...
int loadTWFileintoStructAndHash(const char *filename, struct WordStructure *instance, struct hashMap *mp, int *count)
{
    FILE *fileptr = fopen(filename, "r");
    char buffer[MAX_LEN];
    (*count) = 0;
...
void loadSWFileintoHash(const char *filename, struct hashMap *mp)
{
    ...
    char buffer[MAX_LEN];
    int i = 0;
    while (fgets(buffer, MAX_LEN, fileptr))
    {
        buffer[strcspn(buffer, "\n")] = 0;
